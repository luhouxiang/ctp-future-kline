...
class MA20StateDiagramShortStrategy(Strategy):  # MA20状态图做空策略实现
    ...
    def _handle_init(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if above:
            ...
            return _no_signal(ctx.request, "close above MA20 baseline ready", self._base_metrics(ctx.state, ctx.ma20, ctx.ma_period), trace, False)
        ...
        return _no_signal(ctx.request, "waiting for close above MA20", self._base_metrics(ctx.state, ctx.ma20, ctx.ma_period), trace, False)
    ...
    def _handle_terminal_reset(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        return _no_signal(ctx.request, "terminal state reset", self._base_metrics(ctx.state, ctx.ma20, ctx.ma_period), trace, False)
    ...
    def _handle_below_ma20(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if stood_above:
            ...
            return _no_signal(ctx.request, "waiting for reclaim confirmation", self._base_metrics(ctx.state, ctx.ma20, ctx.ma_period), trace, False)
        ctx.state.reclaim_above_bars = 0
        if touched:
    ...
    def _handle_rebound_to_high(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if stood_above:
            ...
            return _no_signal(ctx.request, "waiting for second break or reclaim confirmation", self._base_metrics(ctx.state, ctx.ma20, ctx.ma_period), trace)
        ctx.state.reclaim_above_bars = 0
        if broke_trigger:
    ...
    def _handle_holding(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if rising_low_take or strong_bull_take:
            ...
            exit_check = (
                self._bar_check(ctx, "触碰MA20后低点上移", True, state.signal_rising_low_bars, settings["profit_rising_low_bars"])
                if rising_low_take
                else self._bar_check(ctx, "强阳站稳MA20", True, ctx.close - ctx.open_price, settings["strong_bull_atr_multiple"] * atr)
            )
            checks = [exit_check]
            ...
            trace = _trace(ctx.request, "bar", TAKE_PROFIT, "止盈", 8, "passed", "trend-following profit exit after MA20 reclaim", checks, metrics, {"target_position": 0, "confidence": 0.8, "signal": "CLOSE_SHORT"})
        ...
        if hit_stop or no_new_low_stop or stood_above_stop:
            ...
            if hit_stop:
                ...
                exit_check = self._bar_check(ctx, "亏损后重新站上MA20", True, ctx.close, ctx.ma20, ctx.close - ctx.ma20)
            checks = [exit_check]
            if holding_state_changed:
                checks.append(self._transition_check(ctx, previous_holding_state, state.state, "holding profit/loss state changed"))
        ...
        if profitable:
            if not state.signal_ma20_touched:
                ...
                current_check = self._bar_check(ctx, "盈利后触碰MA20，等待反弹达标", True, rebound, settings["profit_rebound_atr_multiple"] * atr)
            else:
                current_check = self._bar_check(ctx, "反弹达标，等待低点上移", True, state.signal_rising_low_bars, settings["profit_rising_low_bars"])
        elif stop_deferred:
