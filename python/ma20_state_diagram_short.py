...
class MA20StateDiagramShortStrategy(Strategy):  # MA20状态图做空策略实现
    ...
    def _update_zigzag_features(self, state: StateDiagramShortState, request: RequestDict) -> None:
        ...
        if not isinstance(peaks, list) or state.current_bar_index is None:
            state.zigzag_recent_peaks = []
            state.zigzag_peak_distances = []
            return
        ...
        for item in peaks[:3]:
            ...
            try:
                ...
                continue
            distance = int(state.current_bar_index) - pivot_index
            peak = {
        ...
        state.zigzag_recent_peaks = recent
        state.zigzag_peak_distances = distances
