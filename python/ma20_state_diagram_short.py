...
class MA20StateDiagramShortStrategy(Strategy):  # MA20状态图做空策略实现
    ...
    def _handle_above_ma20(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if not ctx.state.above_ready:
            checks = [self._bar_check(ctx, "整根K线在MA20之上", full_above, ctx.close, ctx.ma20, ctx.close - ctx.ma20)]
    ...
    def _handle_below_ma20(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if stood_above:
            ...
            checks = [
                self._bar_check(ctx, "重新站上MA20", True, ctx.close, ctx.ma20, ctx.close - ctx.ma20),
    ...
    def _handle_rebound_to_high(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if broke_trigger:
            ...
            checks = [
                self._bar_check(ctx, "跌破触碰K开收低价", broke_trigger, ctx.low, ctx.state.trigger_line),
                self._transition_check(ctx, REBOUND_TO_HIGH, SECOND_BREAK_SIGNAL, "SHORT: broke touch bar open/close low"),
            ]
        ...
        checks = [self._bar_check(ctx, "等待二次跌破触发线", True, ctx.low, ctx.state.trigger_line)]
    ...
    def _handle_second_break_signal(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        next_state = PROFIT_HOLDING if ctx.close < ctx.open_price else LOSS_HOLDING
        checks = [
            self._bar_check(ctx, "下一根K线开盘确认持仓", True, ctx.open_price, "entry open"),
            self._transition_check(ctx, SECOND_BREAK_SIGNAL, next_state, "short position confirmed at next bar open"),
        ]
