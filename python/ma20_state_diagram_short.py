...
from strategy_common import (
    ...
    _check,
    _event_ts,
    _feature_score_metric_fields,
    _feature_score_summary,
    _float,
    _instance_id,
    _instance_mode,
...
class MA20StateDiagramShortStrategy(Strategy):  # MA20状态图做空策略实现
    ...
    def _bar_trace(self, ctx: StateDiagramBarContext, step_key: str, label: str, index: int, status: str, reason: str, checks: list[CheckDict], extra_metrics: dict[str, Any] | None = None) -> dict[str, Any]:
        # 生成单根K线在状态图中的执行轨迹。
        metrics = self._base_metrics(ctx.state, ctx.ma20, ctx.ma_period, ctx.ma60)
        if extra_metrics:
            metrics.update(extra_metrics)
        return _trace(ctx.request, "bar", step_key, label, index, status, reason, checks, metrics)
    ...
    def _handle_rebound_to_high(self, ctx: StateDiagramBarContext) -> ResponseDict:
        ...
        if broke_trigger:
            ...
            score_summary = _feature_score_summary(ctx.request)
            score_metrics = _feature_score_metric_fields(score_summary)
            ...
            score_blocked = bool(score_summary.get("enabled") and not score_summary.get("passed", True))
            if ma20_slope_blocked or ma60_slope_blocked or peak_blocked or score_blocked:
                checks = [
                    ...
                    self._bar_check(ctx, "入场指标加权评分过滤", not score_blocked, score_summary.get("total_score"), score_summary.get("feature_score_min")),
                    self._transition_check(ctx, REBOUND_TO_HIGH, INIT, "entry filter blocked short"),
                ]
                self._reset_setup(ctx.state)
                trace = self._bar_trace(ctx, INIT, "初始待机", 1, "failed", "entry filter blocked short", checks, score_metrics)
                metrics = self._base_metrics(ctx.state, ctx.ma20, ctx.ma_period, ctx.ma60)
                metrics.update(score_metrics)
                return _no_signal(ctx.request, "entry filter blocked short", metrics, trace)
